...
import { FaHome, FaBuilding, FaTree, FaHotel, FaStarAndCrescent } from 'react-icons/fa';
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {categories.map((category, index) => (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}    // Scale effect on tap (useful for mobile devices)
              onClick={() => handleCategoryClick(category)}
            >
              {/* Category icon */}
              <IconContext.Provider value={{ color: "gray", size: "3rem" }}>
                <div className="w-16 h-16 mb-2 flex items-center justify-center text-gray-700 shadow hover:shadow-lg transition-shadow duration-300">
                  <category.icon />
                </div>
