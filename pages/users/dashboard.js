...
const Dashboard = () => {
    ...
    const handleLogout = () => {
        signOut();  // Calling signOut method from context
        router.push('/users/login'); // Redirect to the login page after logout
    };

    useEffect(() => {
        if (!user && !authLoading) {
            // Redirect to the login page if not loading and no user is logged in.
